...
function loadNav()
{
    ...
    <nav class='navbar navbar-expand-lg navbar-dark bg-dark fixed-top'>
      <div class='container'>
        <a class='navbar-brand' href='index.php'>New Order</a>
...
function loadFoot()
{
    if(($_SERVER['PHP_SELF'] == '/restaurant/login.php')) $pos = 'absolute';
    else $pos = 'relative';
    echo "</div>
        <!-- /.row -->
        </div>
...
function loadLogin()
{
    echo "<form action='javascript:void(0)' id='login-form' style='margin: auto; width: 35%; text-align: center;'>
    ...
    <form action='javascript:void(0)' id='register-form' style='margin: auto; width: 35%; text-align: center;'>
...
function loadPreviousOrders($selectedDate)
{
    ...
    //Connect to DB
    $conn = sqlConnect();
    $sql = "SELECT id,menu_item_quantities,price,deliv_cost,mode,server,customer_id FROM orders WHERE date = '$selectedDate';";
...
function loadMenu($cat)
{
    ...
    if($cat != 'other')
    {
        ...
        $sql="SELECT id,name,price_in,price_out,meat,img FROM menu_items WHERE category = \"$cat\" GROUP BY name;";
        $result = mysqli_query($conn,$sql);
...
function loadOrderList()
{
    ...
    if(isset($_SESSION['order']))
    {
        ...
        if(isset($_SESSION["mode"])) {
            $mode = $_SESSION["mode"];
            echo "<div class='a list-group-item h6'>Customer Dining: $mode</div>";
        }
